...
export function ColorLegendPanel({ min, max, label, idSuffix }: ColorLegendProps) {
  ...
  return (
    <figure style={{ margin: '0 0 18px 0', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gridArea: '14 / 1 / 14 / 13' }}>
      <figcaption style={{ fontSize: 13, color: COLORS.text, marginBottom: 4, textAlign: 'center' }}>{label}</figcaption>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <svg width="240" height="18" aria-label={label} role="img" style={{ display: 'block', margin: '0 auto' }}>
          <defs>
